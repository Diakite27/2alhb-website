...
function SectionCard({ section, sectionLabel, index }: { section: OfficialDocumentSection; sectionLabel: string; index: number }) {
...
export default function StatutsPage() {
  const { data: doc, loading } = useApiData(
    () => api.getOfficialDocument("statuts"),
  ...
  return (
    <>
      <PageHeader
        title={doc?.title || "Statuts de la 2ALHB"}
        subtitle={doc?.subtitle || "Le cadre juridique et organisationnel qui structure notre association."}
        breadcrumbs={[
            ...
            <>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-10 p-5 bg-white dark:bg-dark-card rounded-2xl shadow-sm">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      <p>Version {doc.version} — Adopté le {doc.adopted_date}</p>
              ...
              <div className="space-y-3 mb-10">
                {doc.sections.map((section, i) => (
                  <SectionCard key={section.id} section={section} sectionLabel={doc.section_label} index={i} />
                ))}
              </div>
